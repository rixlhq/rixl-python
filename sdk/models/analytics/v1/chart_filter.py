from __future__ import annotations
from collections.abc import Callable
from dataclasses import dataclass, field
from kiota_abstractions.serialization import Parsable, ParseNode, SerializationWriter
from typing import Any, Optional, TYPE_CHECKING, Union

if TYPE_CHECKING:
    from .chart_filter_operator import ChartFilter_operator

@dataclass
class ChartFilter(Parsable):
    # The field property
    field: Optional[str] = None
    # The operator property
    operator: Optional[ChartFilter_operator] = None
    # The values property
    values: Optional[list[str]] = None
    
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> ChartFilter:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: ChartFilter
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return ChartFilter()
    
    def get_field_deserializers(self,) -> dict[str, Callable[[ParseNode], None]]:
        """
        The deserialization information for the current model
        Returns: dict[str, Callable[[ParseNode], None]]
        """
        from .chart_filter_operator import ChartFilter_operator

        from .chart_filter_operator import ChartFilter_operator

        fields: dict[str, Callable[[Any], None]] = {
            "field": lambda n : setattr(self, 'field', n.get_str_value()),
            "operator": lambda n : setattr(self, 'operator', n.get_enum_value(ChartFilter_operator)),
            "values": lambda n : setattr(self, 'values', n.get_collection_of_primitive_values(str)),
        }
        return fields
    
    def serialize(self,writer: SerializationWriter) -> None:
        """
        Serializes information the current object
        param writer: Serialization writer to use to serialize this model
        Returns: None
        """
        if writer is None:
            raise TypeError("writer cannot be null.")
        writer.write_str_value("field", self.field)
        writer.write_enum_value("operator", self.operator)
        writer.write_collection_of_primitive_values("values", self.values)
    

