...
Here is the whole thing. Build a client, then list the images in a project:
...
Minting needs no credentials of its own, since the ID and secret are in the body, so
...
**Account management**: `client.auth`, plus `client.organizations`. Sign-in flows
...
Response fields are optional too, since the API omits what it has nothing to say
about, so guard with `or []` and `is not None` rather than assuming a value is there.
...
failing status, so switch on `response_status_code` rather than on the exception
